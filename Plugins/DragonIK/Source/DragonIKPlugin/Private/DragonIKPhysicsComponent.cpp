	...
	if (handler_holder != nullptr && owning_skeleton != nullptr && component_grabbing_state == true)
	{
		if (handler_holder->physics_handle_array.Num() > 0)
		{
			...
			for (int handle_i = 0; handle_i < handler_holder->physics_handle_array.Num(); handle_i++)
			{
				...
				if (handler_holder->physics_handle_array[handle_i] != nullptr && Raw_Animation_Transforms.Num() > 0)
				{
...
void UDragonIKPhysicsComponent::UpdateBodyBlends()
{
	...
	*/
	...
	if (owning_skeleton != nullptr && handler_holder != nullptr && use_auto_blending_logic == true && is_released == false)
	{
		...
		for (int i = 0; i < Bodies_Physics_Blend.Num(); i++)
		{
			...
			*/
			...
			Bodies_Physics_Blend[i] = FMath::Clamp<float>(Bodies_Physics_Blend[i], 0, 1);
		...
		*/
		...
		owning_skeleton->ForEachBodyBelow(Custom_Root_Bone, true, false, [=](FBodyInstance* BI)
		{
			...
			BI->PhysicsBlendWeight = Bodies_Physics_Blend[bone_index];
		...
		if (modify_physics_states == true)
		{
			owning_skeleton->ForEachBodyBelow(Custom_Root_Bone, true, false, [=](FBodyInstance* BI)
			{
				...
				{
					if (BI->bSimulatePhysics == false && Bodies_Physics_Blend[bone_index] > 0)
					{
						BI->SetInstanceSimulatePhysics(true);
					}
				}
				...
				{
					if (BI->bSimulatePhysics == true && Bodies_Physics_Blend[bone_index] == 0)
					{
						BI->SetInstanceSimulatePhysics(false);
		...
		for (int i = 0; i < Bodies_Physics_Blend.Num(); i++)
		{
			if (Bodies_Physics_Blend[i] > 0 && Physics_States[i] == false)
			{
		...
		if (blending_unwanted_bodies_completed && ragdoll_mode)
		{
			...
			owning_skeleton->ForEachBodyBelow(Custom_Root_Bone, true, false, [=](FBodyInstance* BI)
			{
				...
				//if (all_grabs_finished)
				{
					...
					if (Physics_States[bone_index] == false && Bodies_Physics_Blend[bone_index] <= 0)
					{
						BI->SetInstanceSimulatePhysics(false);
